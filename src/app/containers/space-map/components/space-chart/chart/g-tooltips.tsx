...
const getWidth = ({ data: { symbol } }: HierarchyCircularNode<PackedCategories>, multiplier = 1) => {
  const multipliedValue = symbol.length <= 3 ? 3.5 : symbol.length;
      ...
      {data?.map(elem => (
        <>
          ...
          <marker
            ...
            viewBox={`0 0 ${getWidth(elem, 10)} 50`}
            markerUnits="strokeWidth"
            markerWidth={elem.data.name.length * 10}
            markerHeight="50">
            <rect width={getWidth(elem, 10)} height="24" rx="12" fill="white" />
            <path
              ...
              fill="white"
              transform={`translate(${getWidth(elem, 5) - 12}, 17) scale(1.5)`}
            />
            <path
              fill="#464646"
